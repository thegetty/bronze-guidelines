...
The application of very thin sheets, known as leaf, is a common technique requiring very little precious metal.[^1] Gold leaf is typically 0.1–1.2 µm thick,[^2] normally 7–11 cm square, although smaller pieces are encountered ({% objlink 'fig-287' %}).[^3] Found as early as the second millennium BCE in Egypt, gold leaf was widely used in the Greek and Roman world ({% objlink 'fig-288' %}), then declined in the West during the medieval period due to the development of mercury gilding, then gained popularity again in the Renaissance.[^4] In Asia it is still used for religious sculptures, and devotees apply gold leaf to Buddha figures as an act of worship ({% objlink 'fig-289' %}).[^5] Leaf may be applied directly to clean metal or attached with adhesive.[^6]
...
-   The prime indicator that leaf has been used is the overlapping of the straight edges, visible as squares on the surface ({% objlink 'fig-287, fig-290' %}). With wear, the overlapping edges are sometimes all that remains because of the greater thickness of material.
...
{% objgroup "3", "fig-287, fig-288, fig-289, fig-290" %}
...
Mercury gilding has been widely used on sculpture ({% objlink 'fig-291' %}), appearing in China as early as the Warring States period (475–221 BCE) ({% objlink 'fig-292' %}).[^12] In Europe, it can be found a little later, in the Late Iron Age and Roman periods, flourishing in the Middle Ages, the Renaissance,[^13] and into the nineteenth century.[^14] Safety legislation brought its use almost completely to an end in Europe, though not in all parts of the world,[^15] by the close of the nineteenth century, due to the toxicity of the mercury vapors. Because of the semiliquid application, it is more suited to gilding complex shapes than foil or leaf. It has also been used to gild selected parts of an object (known as parcel gilding) ({% objlink 'fig-293' %}).
...
-   Amalgam gilding is applied as a semiliquid, so may show splashes or runs ({% objlink 'fig-293, fig-294' %}), and adherence to the bronze substrate is good, though the gilding may show wear at high points.

-   The thickness of the gilding layer can vary from 1 to 10 µm.[^18]

-   A metallographic cross section of mercury gilding will show good bonding to the bronze substrate ({% objlink 'fig-295' %}).
...
{% objgroup "3", "fig-291, fig-292, fig-293, fig-294, fig-295" %}
...
-   Visible indicators of electroplating include loss of the thin plating due to wear, and also blistering of the plating layer due to the substrate being imperfectly cleaned or the electroplating bath not being adequately maintained ({% objlink 'fig-296' %}).
...
Mechanical attachment of metal foil is one of the earliest methods ({% objlink 'fig-297' %}).[^27] Foil is generally defined as a thin sheet of metal formed by hammering or rolling, and unlike metal leaf, it is thick enough to support its own weight when held at the edge. A sheet of metal foil can be wrapped around an object and held in place by folding and crimping the joints. Alternatively, it can be fixed by rivets or by {% def "punch" "punching" %} the edges of the sheet into grooves that are hammered shut or filled with {% def "soldering" "solder" %} ({% objlink 'fig-298' %}). Inserting gold foil into grooves is a method mentioned by Pliny for a sculpture of the young Alexander.[^28] Rubbing thin gold or silver sheet into fine grooves or punch marks is a method used on ironwork and copper alloys.[^29]

*Identification*

-   Foil gilding tends to blur the detail of a casting ({% objlink 'fig-299' %}) and can be detected by the presence of wrinkles or folded overlaps in the plating layer, or the presence of grooves in the casting ({% objlink 'fig-300' %}).
...
{% objgroup "3", "fig-297, fig-298, fig-299, fig-300" %}
...
-   As it is applied molten, it has the typical microstructure of a cast alloy (see [I.2§3.1](/vol-1/2/#s3-1)).
...
-   Silvery surfaces are sometimes found on cast bronzes caused by a casting phenomenon known as tin sweat or inverse segregation ({% objlink 'fig-163' %}).[^51] This may be an accident of casting but could be deliberately encouraged to create a silvery surface on bronze.
...
The flowchart in [figure 304](/visual-atlas/304/){target=object-iframe} will guide you in the visual inspection of plating (see also {% objlink 'fig-305' %}). For more details on how to identify plating processes, refer to the *Identification* sections in this chapter, which describe all potential processes. For details on analytical techniques see {% objlink 'table-05, table-10' %}, and [II.6](/vol-2/6/).
...
Keep in mind that plating is often renewed and restored during the lifetime of a sculpture ({% objlink 'fig-289' %}). Traces of lost or hidden metal plating are potentially detectable, and original and later interventions are sometimes distinguishable (see [I.7§2.7](/vol-1/7/#s2-7)).

{% objgroup "3", "fig-304, fig-305, fig-289" %}
...
[^3]: For a brief introduction to contemporary practices in leaf gilding see: https://www.philamuseum.org/learn/conservation/case-study-gilding-diana.
...
[^11]: {% cite "d’Arcet 1818" %}; {% cite "Cellini [1568] 1967" %}.
...
[^20]: {% cite "d’Arcet 1818" %}; {% cite "Jett 1993" %}; {% cite "Strahan and Maines 2000" %}; {% cite "Cowell, La Niece, and Rawson 2003" %}.

[^21]: {% cite "Oddy, Bimson, and La Niece 1981" %}.

[^22]: {% cite "La Niece 1990" %}.

[^23]: {% cite "Hunt 1973" %}; {% cite "Raub 1993" %}; {% cite "Lins 2000" %}.
...
[^31]: See {% cite "Bott and Willer 2014" %}; {% cite "Willer, Schwab, and Mirschenz 2017" %}.
...
[^35]: {% cite "Lechtman 1984" %}; {% cite "Scott 1983" %}; {% cite "Scott 2000" %}.
...
[^39]: {% cite "Northover and Salter 1990" %}; {% cite "La Niece 1993" %}.
