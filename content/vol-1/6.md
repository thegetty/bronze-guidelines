...
Marks may have been added during the fabrication process to serve as aids in assembly (see [I.5](/vol-1/5/))[^10] or registration; as votive inscriptions ({% ref 'fig-020, fig-257' %}); or to identify the model or title of the piece ({% ref 'fig-258' %}), the owner (for instance for an inventory number; {% ref 'fig-259' %}), the commissioner, the artist ({% ref 'fig-260' %}), the process ({% ref 'fig-261' %}), the {% def "edition" %} number ({% ref 'fig-261' %}), the date, and/or the founder or foundry ({% ref 'fig-014, fig-262, fig-263, fig-264' %}).[^11]
...
See {% ref 'table-05' %} for a summary of available techniques. See {% ref 'table-13, table-10' %}, and associated chapters in volume II for details of the techniques.
