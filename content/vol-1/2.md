...
Adding different metallic elements to copper makes it possible to control the resulting alloy’s properties (for instance {% def "castability" %}, strength, resistance to corrosion, workability) as desired by the sculptor or {% def "founder" %} (see {% ref 'table-06' %}, also [I.2§2](/vol-1/2/#S2) below). Cultural influences, the cost of raw materials, the desired color, and/or the need for the {% def "cast (n.)" "cast" %} to take {% def "gilding" %} may also play a role in the choice of alloy. Historically, except for {% def "brass" %}, copper alloys were produced by combining the different metals in a crucible or furnace at the time of the {% def "pour" %}.[^1] Increasingly, commercial art foundries have tended to acquire pre-alloyed ingots.
...
Metallography also provides excellent evidence of the presence or absence of cold and/or hot working.[^27] Theoretically, it might also provide evidence of conditions during casting.[^28] Metallographic sampling is not readily done on relatively pristine works, especially since the areas that would be most interesting to sample tend to be on the outer surface, in clearly visible areas. Less invasive methods are available, but there are limitations to their ability to answer certain questions (see nondestructive testing in [II.2](/vol-2/2/)).
