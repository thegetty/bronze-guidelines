...
A simple spreadsheet may already constitute a database as soon as all these requirements are met. A number of databases related to cultural heritage may be used as examples.[^8] The Hephaistos database is to date the only example of a large technical database dedicated exclusively to bronze sculpture ({% ref 'fig-368' %}).[^9]
