...
Broadly speaking, trapped-charge dating is an accurate method but not a precise one. For instance, the uncertainty of a TL or OSL date may be on the order of +/-140 years for a thousand-year-old object with 95% confidence ({% ref 'fig-413' %}). It is possible to reduce this uncertainty by increasing the number of measured samples, particularly where all the parameters are well characterized and measured. Recently, earth scientists have favored single-crystal OSL whereby each mineral grain from a sample is analyzed individually, allowing for a more nuanced interpretation of results.[^7]
...
Currently, standard practice is to extract at least one cubic centimeter of un-corroded iron for analysis. Sampling is usually executed using a thin diamond cutting wheel (0.6–1 mm thick; {% ref 'fig-436' %}) This sample should be examined metallographically and then resampled to yield between 125–1000 mg (according to the carbon content in the metal) of iron drillings suitable for analysis.
