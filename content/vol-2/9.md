...
Historical simulations focused broadly on the fabrication of bronze sculpture have been carried out in European classical[^4] and Renaissance[^5] contexts. A long-term study of the economic factors affecting medieval European {% def "brass" %} making was published in 2018.[^6] More specific studies have focused on fusion {% def "welding" %},[^7] sixteenth-century French casting recipes,[^8] and the {% def "patina" %} formulations of the Renaissance sculptor Antico (Italian, ca. 1455–1528).[^9] A simple but effective experiment to determine whether tool marks observed on a bronze sculpture were made in the wax {% def "model" %} or directly on the metal was conducted by Lorenzo Morigi ({% objlink 'fig-271' %}).[^10]

For researchers interested in historical simulation, publications beyond the scope of bronze sculpture may provide guidance and inspiration. Several archaeometallurgical publications address closely related subject matter.[^11] Two proceedings focused on paintings by the Art Technological Source Research study group discuss various aspects of using historic texts to inform historic reconstructions.[^12]

{% objgroup "3", "fig-271" %}

{% endaccordion %}
{% accordion "### 1.2 Laboratory simulation" "s1-2" %}
...
Examples of recent laboratory simulations include fusion welding experiments[^13] that have yielded evidence of the quite narrow operating window (alloy, temperature, timing) in which fusion welding can be carried out ({% objlink 'vid-12' %}). These results have highlighted the exceptional skill of classical European {% def "founders" %} in controlling complex heat transfers, particularly for large bronzes where welded joins may extend over several meters (see [I.5](/vol-1/5/), [Case Study 1](/case-studies/1/)). The {% def "castability" %} of various copper alloys has been tested through a series of experimental simulations that have, for the first time, properly demonstrated the role of lead in promoting the successful making of thin-walled {% def "cast (n.)" "casts" %} ({% objlink 'fig-437, fig-438' %}).[^14] Detailed investigations into specific patina formulations in classical bronzes have shed additional light on the sophistication of ancient craftspeople,[^15] and the thermodynamics of brass production by cementation have been rigorously investigated.[^16]
...
{% objgroup "3", "vid-12, fig-437, fig-438" %}

{% endaccordion %}
{% accordion "### 1.3 Virtual simulation" "s1-3" %}
...
[^4]: {% cite "Formigli 1993" %}; {% cite "Formigli and Hackländer 1999" %}.
...
[^13]: {% cite "Azéma et al. 2011" %}; {% cite "Azéma et al. 2013" %}.

[^14]: {% cite "Pernot 2000" %}; {% cite "Andrieu et al. 2000" %}; {% cite "Mille 2017" %}; see also [I.2§2.2](/vol-1/2/#s2-2).

[^15]: {% cite "Mathis et al. 2007" %}.

[^16]: {% cite "Newbury, Notis, and Newbury 2005" %}; {% cite "Bourgarit and Bauchau 2010" %}.
