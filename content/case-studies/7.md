...
In casting up the various wax sections, I used two very different methods of sprueing, “top fed” and “bottom fed.” This is simply because some sections present better opportunities for one method over another. For example, the head, legs, and tail can be considered irregular cylinders. By pouring a plaster core into these and placing feeds (the sprues used to direct the bronze into the mold) at the top (**video 2**), they make for very straightforward castings. The panels of the body, on the other hand, are sprued up while still in the mold so as not to cause any distortion in the geometry of the wax. They can be sprued up from the top, but this tends to cause overheating and excessive {% def "porosity" %} at that location. By bottom feeding them, the flow of metal is less turbulent and the heat is dispersed throughout the mold.
...
I usually remove as much core as possible, but often some small fragments of material may be left inside. In the case of the double core method, a thin line or minimal {% def "flashing" %} would be visible inside the bronze where the two cores meet (**video 11**).

## <span class="cs-category">8 The investment</span> key for a good-quality and clean cast surface
...
## <span class="cs-category">12 Chasing</span> different from one edition to another
...
## <span class="cs-category">13 Patina</span> unique for each edition
