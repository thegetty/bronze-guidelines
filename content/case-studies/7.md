...
This choice of making the mold in small discrete sections will be invisible from the outside, as I can fix whatever evidence there is. However, thin lines can be seen between each on the inside where the sections have been welded together. The pattern of lines in {% annoref fig='fig-545' text='the X-ray image' onscroll='true' %} will describe the way I planned the mold and therefore how I chose to overcome future difficulties in the casting process. Additional lines may be found where wax sections needed to be further cut down, which we see in the joining of the head and neck of the horse.
...
Making wax sections is generally easy, but the technique changes from mold to mold depending on its design. For example, the head and neck of the horse were {% annoref fig='fig-021' text='made in a two-part mold split' onscroll='true' %} in half along its length that forms an open vessel shape. It is therefore easy to pour in molten wax and slush it around the inside of the mold ({% annoref fig='vid-02' text='video 2' %}). This was also true of the legs and tail. But the body was molded in two large halves divided lengthwise along the horse’s back and belly and designed so that each half would be cast separately. Here I painted wax into the open half mold because that was easier in this case.
...
In casting up the various wax sections, I used two very different methods of sprueing, “top fed” and “bottom fed.” This is simply because some sections present better opportunities for one method over another. For example, the head, legs, and tail can be considered irregular cylinders. By pouring a plaster core into these and placing feeds (the sprues used to direct the bronze into the mold) at the top ({% annoref fig='vid-02' text='video 2' %}}), they make for very straightforward castings. The panels of the body, on the other hand, are sprued up while still in the mold so as not to cause any distortion in the geometry of the wax. They can be sprued up from the top, but this tends to cause overheating and excessive {% def "porosity" %} at that location. By bottom feeding them, the flow of metal is less turbulent and the heat is dispersed throughout the mold.
...
In the interior of this bronze sculpture you should see some evidence of the different sprueing choices, but reconstructing the way a bronze has been sprued based on remaining sprue stubs, residual flow patterns of the dross, or oxidized metal in the cast surface ([I.3](/vol-1/3/)), et cetera, would be difficult at best. And of course, if I had chosen to place the sprues on the outer, modeled surface of the wax, then no evidence would be left after I reworked the locations of the cut-off {% annoref fig='fig-088' text='sprues' onscroll='true' %} to blend in with the surrounding areas.
...
I have developed the habit of {% annoref fig='fig-547' text='using carpet tacks' onscroll='true' %} as {% def "core supports" %}. I used to use stainless steel rods, but they are expensive and can be difficult to get hold of. Carpet tacks, on the other hand, are very cheap and sold at any hardware store. The design of the tack is also perfect for the task. They are thin with sharp points, perfect for piercing the wax, and have flat heads, which assists in their removal after casting.
...
I usually remove as much core as possible, but often some small fragments of material may be left inside. In the case of the double core method, a thin line or minimal {% def "flashing" %} would be visible inside the bronze where the two cores meet ({% annoref fig='vid-11' text='video 11' %}).
...
My recent experiments in re-creating one of the Rothschild Bronzes ({% cite "Lacey 2018" %}) have revealed how much care and attention are needed in applying the first layers of investment. This I already knew, but I found that I could use a historic recipe of fine clay mixed with ground inert powders for the first coats and back them up with ceramic shell ({% annoref fig='vid-02' text='video 2' %}), or just use ceramic shell for the less detailed parts. This made for {% annoref fig='fig-007' text='very lightweight molds' onscroll='true' %} that captured detail accurately, but were easier to remove from the bronze than ceramic shell. The clay layers are also cheaper than the zirconium silicates typically used for the initial layers in ceramic shell investments. It was through these experiments that I developed a method of investing the horse that worked best for me.
...
These new mixed techniques that I’ve been exploring with historical materials will rarely leave a trace on the finished bronze. Possibly some of the fine particles may remain lodged in crevices. Potentially, chemical analysis could pick this up. {% annoref fig='vid-11' text='' onscroll='true' %}
...
I do use electric tools, for example grinders and pendant drills, but mostly I use hand tools like {% def "punches" %}, chisels, and files, the majority of which I have made myself to suit my needs ({% annoref fig='vid-11' text='video 11' %}). My use of hand tools is not out of a sense of tradition or to uphold craft values. I’m very physical and love to express this through the bronze, and these tools allow me to communicate with the metal in an ongoing dialogue. That invigorates me, and hopefully both the sculpture and I have a near-tireless energy for such work.
